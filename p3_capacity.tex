...
reports the answer twice: once at the model level, as a reader would compute it from the released table, and
then again under clustering on families, at which point \emph{neither} non-tautological
geometric predictor survives. Section~\ref{sec:notmech} reports a head-to-head against a competing mechanism,
which our geometry \emph{loses}. Section~\ref{sec:confounds} documents three measurement artifacts in the
declaration-site geometry, each strong enough to invert the qualitative answer.

\paragraph{What this paper claims, and what it does not.} It claims an observational regularity over real
models: \emph{binding capacity is a recipe$\,\times\,$scale interaction set primarily by the pretraining recipe, not by scale alone}. It does
...
measure at the declaration site is a property of the \emph{entity code}, not of the binding, and the paper
names it accordingly.

\paragraph{Two exclusions, stated before any number.} \emph{(i)} $\kstar$ is defined as a threshold crossing
...
\paragraph{Task.} Each trial declares $K$ bindings ``\texttt{$e_i$: $o_i$}'' (entity $e_i$, single-token
obligation $o_i$, from disjoint pools), then an interference block of $D$ distractor tokens, then a recall
query ``\texttt{The task for $e_j$ is:}'' for a random $j$. The model is correct iff its top obligation-token
logit is $o_j$. Varying $K$ probes \emph{capacity}; varying $D$ probes \emph{interference-robustness}. Chance
is $1/|\mathcal{O}|=\Chance$.

\paragraph{The capacity statistic, and its hazard.} $\kstar$ is the smallest $K$ at which recall falls below
the midpoint of the $K=1$ ceiling and chance. It is a threshold crossing of a smooth curve, and thresholded
...
so we cannot rule out the thresholding artifact from it directly. We mitigate it in three ways instead: by
excluding any model whose $K=1$ ceiling is near chance (its threshold is noise), by reporting the
\emph{median} difference rather than the range, and by clustering on families. The full curves are not in the
released table; releasing them is the outstanding fix, and the re-measurement runner ships with the paper's
code (\texttt{analysis/ksweep.py}, with \texttt{analysis/compare\_ksweep.py} as the agreement check).
...
\paragraph{Models.} The released capacity table holds \NModelsRaw{} open models from ten families. \emph{Old recipe}
($n=\NOldRaw$): GPT-2 (M/L/XL), GPT-Neo (125M, 1.3B), OPT (350M, 1.3B, 2.7B, 6.7B), BLOOM (560M, 1.7B), Cerebras-1.3B,
...
recipe$\,\times\,$scale interaction, not a scale law. The Qwen ladder is \emph{not monotone}: the 3B
model reaches the censored ceiling and the 7B model sits below it.
Alignment leaves capacity untouched: the OLMo-2 base/SFT/DPO/Instruct ladder holds $\kstar=\KstarOlmoLadder$ at
every stage: quantifying, on a capacity threshold, the no-consistent-alignment-benefit result that
\citet{kim2405} established for entity tracking.
...
large difference in \emph{central tendency} (median $\KstarOldMed$ versus $\KstarModMed$), not a clean separation, and the
family-clustered interval reflects that.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{fig5_capacity.pdf}
  \caption{\textbf{Binding capacity $\kstar$ tracks recipe, not scale.} Median $\kstar$ is $\KstarOldMed$ for old
...
\section{Looking for a geometric mechanism, and not finding one}\label{sec:geometry}
...
comparisons is significant at the model level. These numbers suggest a simple picture: old
recipes cram bindings onto overlapping directions and modern recipes spread them, which would connect binding capacity to
the superposition regime of \citet{superposition2505}, where \emph{loss} scales inversely with model dimension
under strong superposition and the degree of superposition is tunable by weight decay, and sit alongside \citet{largercap2605}, who attribute larger models' advantage to reduced gradient
interference on rare tasks.

\paragraph{Thirty models are not thirty independent samples.} They are \emph{ten families}, and Pythia and
OLMo contribute six apiece. A model-level test treats siblings as independent draws. Re-running every
comparison with a bootstrap clustered on families, and with a family-level test, the picture does not hold.
...
mechanism}. The model-level $p$-values are the ones a reader will compute from the released table; we print
them, and then print what the clustering does to them. With ten families (six old, four modern), this is a study with less power than thirty points suggests, and the geometry is where that
shows.
...
\begin{figure}[t]
  ...
  families} (Section~\ref{sec:geometry}); the panels show the model-level pattern in the released table, not a
  mechanism.}
  \label{fig:recipe}
\end{figure}

\section{What this geometry does not explain}\label{sec:notmech}
It is tempting to read the packing story as a competitor to the positional-mechanism account of
\citet{mixing2510}, who attribute multi-binding retrieval failure to a positional circuit that grows noisy as
bindings accumulate. We ran the comparison on the same trials, regressing per-trial error on serial position
and on the pairwise overlap of the binding vectors. \textbf{The geometry loses.} Once position is controlled,
...
\emph{reflexive} one and reaches $95\%$ agreement with next-token distributions; our head-to-head tests only
their positional term against our overlap term, and it is the positional term alone that wins.

So: \emph{across-model} capacity tracks packing efficiency; \emph{within-model, per-trial} error does not. We
report both; the cross-model correlation is an observational regularity over \NModels{} models,
not a per-trial mechanism.

A positional pointer degrades, which is why
per-trial error tracks serial position. But on exactly those failures a linear probe still recovers the correct
binding. What degrades is the \emph{pointer}; the bound \emph{content} survives. One caveat on the comparison
itself: our error curve is a primacy curve rather than the U-shape of \citet{mixing2510}, because a $\Dblock$-token
...
degrades as the distractor block $D$ grows at fixed $K$) are distinct axes. Figure~\ref{fig:robust} sweeps $D$
at $K=\RobustK$ for the eight models with committed curves. Recall at $D=0$ already spans $\RobustDzeroLo$ to
$\RobustDzeroHi$ (median $\RobustDzeroMed$), and the split is exactly the capacity split: the \RobustLowKN{}
...
$\RobustDendMed$), and the collapse \emph{rates} do not follow the capacity ranking. A model can therefore top
the capacity ranking and be unremarkably fragile, or the reverse. This dissociation matters for the interference wall reported by
...
Each of these artifacts produces a wrong but plausible-looking number, and each is easy to walk into. They
get a section rather than a footnote.
...
Both confounds are \emph{installed by pretraining}. Across Pythia-1.4B checkpoints the positional variance
...
the binding. We verified this rather than assuming it: entity decodes at $1.000$, obligation at chance.
...
We flag this prominently because no downstream figure exposes it. It surfaced only when projecting the
declaration-site residuals onto the obligation classes produced no separation in any frame, including a
supervised one, which sent us to check the token position. A
further provenance caveat: the plotting generator for Figure~\ref{fig:recipe} now exists and reads a
...
models; \citet{binding2310} identify the binding-ID mechanism whose fidelity \emph{improves} with scale, an apparent tension with our flat ladder that we leave explicitly open: a mechanism's fidelity and a capacity
ceiling can move independently, and the dissociation experiment is the natural next measurement. Two recent
...
because the from-scratch models were undertrained. We report the observational law and mark the causal knob as
open.
...
declaration-site vector cannot carry the binding at all. What remains after those corrections, with the ten
families clustered, is a capacity law that survives, a subspace that encodes entities rather than their
bindings, and two geometric correlates whose confidence intervals touch zero.
